...
@section('content')
    ...
    <main class="col-md-9 ms-sm-auto col-lg-10 px-md-4">
        <h1 class="mt-4">Table featured-list</h1>
        <button class="btn btn-primary float-right" onclick="window.location.href='/datatables/list/featured/create'">+
            Create</button>
    ...
    <script>
        $(document).ready(function() {
            var table = $('#myTable').DataTable({
                ...
                "ajax": "{{ route('api.datatables.featured.data') }}",
                "columns": [{
                        "data": "id",
                        "title": "ID"
                    },
                    ...
                    },
                    {
                        ...
                        "title": "เนื้อหา",
                        "render": function(data, type, row) {
                            // ตรวจสอบว่าเนื้อหามีความยาวมากกว่า 50 ตัวอักษรหรือไม่
                            if (data.length > 20) {
                                // ถ้ามีให้แสดงเนื้อหาเฉพาะ 50 ตัวอักษรแรก
                                var shortContent = data.substring(0, 20) + '...';
                                // ใส่ Tooltip ที่มีเนื้อหาทั้งหมด
                                return '<span data-bs-toggle="tooltip" data-placement="top" title="' +
                                    data + '">' + shortContent + '</span>';
                  ...
                  
                    {
                        ...
                        "title": "ลบ",
                        "render": function(data, type, row) {
                            return '<form action="{{ url('/delete/list/featured/') }}/' + row.id +
                                '" method="post">' +
                                '@csrf' +
                                '@method('DELETE')' +
        ...
        function editProfile(id) {
            ...
            window.location.href = '/datatables/list/featured/edit/' + id;

        }
        function confirmDelete(id) {
            if (confirm('Are you sure you want to delete this item?')) {
                // ส่งคำร้องขอลบไปยังเซิร์ฟเวอร์
                fetch('{{ url('/delete/list/featured/') }}/' + id, {
                        method: 'DELETE',
                        headers: {
