...
@section('content')
            ...
            <main class="col-md-9 ms-sm-auto col-lg-10 px-md-4">
                <h1 class="mt-4">Table Experience</h1>
                <button class="btn btn-primary" onclick="window.location.href='/datatables/experience/create'">+ Create</button>
...
@section('script')
    ...
    <script>
            ...
            var table = $('#myTable').DataTable({
                ...
                "columns": [{
                    ...
                    },
                    {
                        ...
                        "title":"แก้ไข",
                        "render": function(data, type, row) {
                            return '<button class="btn btn-warning" onclick="deleteProfile(' + row
                                .id + ')">ลบ</button>';
                        }
                    }
                //     {
                ...
                //     "title":"ลบ",
                //     "render": function(data, type, row) {
                //         return '<form action="{{ url('/delete/experience/') }}/' + row.id +
                //             '" method="post">' +
                //             '@csrf' +
                //             '@method('DELETE')' +
        ...
        function editProfile(id) {
            ...
            window.location.href = '/datatables/experience/edit/' + id;

        }

        function deleteProfile(id) {
    // ตรวจสอบความยืนยันจากผู้ใช้ก่อนลบโปรไฟล์
    if (confirm('Are you sure you want to delete ?')) {
        // ดึงค่า CSRF token จาก meta tag
        ...
        fetch(`/delete/experience/${id}`, {
            ...
            headers: {
                ...
                'X-CSRF-TOKEN': csrfToken // ใช้ค่า CSRF token ที่ได้จาก browser
            },
            // จัดการกับคำตอบ
        })
        .then(response => {
        ...
        .catch(error => {
            alert('An error occurred while deleting e.');
            console.error(error);
        });
    }
}
